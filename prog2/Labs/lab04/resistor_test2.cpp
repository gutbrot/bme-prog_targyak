...
int main() {
  ...
  TEST(Resistor, HazibanAdottR) {
    double d = r.getR();
    EXPECT_EQ(107., d) << "Jporta feladatban megadott default ertekkel hasonlitson!" << std::endl;
  } END
  ...
  TEST(ResistorTest, getI) {				// getI
    const Resistor r0(2);                   //konstruktor
  ...
  TEST(ResistorTest, getU) {				// getU
    const Resistor r0(12.12);               //konstruktor
  ...
  TEST(ResistorTest, EQ) {				    // operator==
    const Resistor r0(12345.6789);          //konstruktor
