...
public class XmlMapLoader {
    ...
    public static Model loadAll(String path) throws Exception {
    ...
    private static World loadMap(Element root) {
        ...
        NodeList mapSection = root.getElementsByTagName("map");
        Element mapElement = (Element) mapSection.item(0);
    ...
    private static void loadEnemies(Element root, World world) {
        ...
        Element enemyList = (Element) enemiesSection.item(0);
        NodeList enemyNodes = enemyList.getElementsByTagName("enemy");
    ...
    private static DependencyGraph loadAbilities(Element root) {
        ...
        NodeList abilitySection = root.getElementsByTagName("abilities");
        if (abilitySection.getLength() == 0) return new DependencyGraph();

        Element abilities = (Element) abilitySection.item(0);
        ...
        for (int i = 0; i < abilityNodes.getLength(); i++) {
            ...
            Ability to = abilityMap.get(toId);
            ...
            for (int r = 0; r < reqs.getLength(); r++) {
                ...
                Ability from = abilityMap.get(fromId);
